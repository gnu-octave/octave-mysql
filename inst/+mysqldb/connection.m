...
classdef connection < handle
  ## -*- texinfo -*-
  ## @deftp {Class} mysqldb.connection
  ## Connection class for a MySQL database connection
  ...
  ## @end table
  ##
  ## Class is created using the mysql function.
  ## @seealso{mysql}
  ## @end deftp
  ...
  methods
    function this = connection (varargin)
      ...
      if nargin < 3
        error ("Expected either datasourcename, username, password or username,password,[properties pairs]");
      endif
      ...
      if nargin == 3
        ...
        for idx=3:2:nargin
          ...
          if strcmp(n, "server")
            if !ischar(v) 
             error ("expected server value to be a string");
            endif
            server = v;
          elseif strcmp(n, "databasename")
            datasource = v;
          elseif strcmp(n, "portnumber")
            if !isnumeric(v) || v <= 0
             error ("expected portnumber to be numeric");
            endif
            portnumber = int32(v);
      ...
      this.dbhandle = __mysql_create__(server, portnumber, username, password, datasource, flags, timeout);
    endfunction
    ...
    function data = sqlouterjoin (this, lefttable, righttable, varargin)
      ...
      ## None
      ##
      ## @seealso{mysql, fetch, sqlinnerjoin}
      ## @end deftypefn
    ...
    function data = sqlinnerjoin (this, lefttable, righttable, varargin)
      ...
      ## None
      ##
      ## @seealso{mysql, fetch, sqlouterjoin}
      ## @end deftypefn
    ...
    function commit (this)
      ...
      ## None
      ## @end deftypefn
      __mysql_commit__(this.dbhandle);
    endfunction
    ...
    function data = sqlfind (this, pattern, varargin)
      ...
      if !isempty(catalog)
        filter{end+1} = ["TABLE_CATALOG LIKE '%" catalog "%'"];
      endif
      if !isempty(schema)
        filter{end+1} = ["TABLE_SCHEMA LIKE '%" schema "%'"];
      endif
  ...
  methods (Access = private)
    function rdata = _run (this, sqlquery, format)
      ...
      data = __mysql_run__(this.dbhandle, sqlquery);
      if nargout > 0
...
%! db = [];
%! try
%!  db = mysqldb.connection("MySQLNative", "root", "matlab");
%! catch
%!   db = [];
%! end_try_catch
%!
%! if !isempty(db)
%! db.execute("CREATE TEMPORARY TABLE TestTable (Id INT NOT NULL PRIMARY KEY, Name VARCHAR(255));");

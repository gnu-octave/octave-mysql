...
classdef rowfilter
  ...
  methods (Static = true, Access = private, Hidden = true)
    ...
    function s = str_constraint(c, opconv = [])
      if nargin == 1
        opconv = @mysqldb.rowfilter.no_op_change;
      endif
      s = "";
      if iscell(c)
        ...
        for x = 1:length(c)
          ...
          s = [s mysqldb.rowfilter.str_constraint(v, opconv)];
        endfor
        ...
        if isfield(c, "value1")
          if !isempty(c.value1)
            s = " ( ";
            s = [s mysqldb.rowfilter.str_constraint(c.value1, opconv)];
          endif
          s = [s opconv(c.operation)];
          s = [s mysqldb.rowfilter.str_constraint(c.value2, opconv)];
          if !isempty(c.value1)
            s = [s " )"];
          endif
        elseif isnumeric(c.value)
          if isempty(c.value)
            if opconv == @mysqldb.rowfilter.sql_op_change
  ...
  methods (Access = public)
	...
	tf = mysqldb.rowfilter(this.vars);
    ...
    function val = and(this, v)
      if isa(v, "mysqldb.rowfilter") == false
        error ("Expected a rowfilter")
      endif
      vars1 = this.vars;
      vars2 = v.vars;
      vars = unique({vars1{:} vars2{:}});
      val = mysqldb.rowfilter(vars);
    ...
    function val = char(this)
      val = mysqldb.rowfilter.str_constraint(this.constraints, @mysqldb.rowfilter.sql_op_change);
    endfunction
  endmethods
endclassdef

%!error <Expected variableNames> mysqldb.rowfilter();
%!error <Unknown or unsupported rowfilter input> mysqldb.rowfilter(1);

%!test
%! rf = mysqldb.rowfilter('Column1');
%! assert(properties(rf), {'Column1'});
%! assert(isempty(rf))

%!test
%! rf = mysqldb.rowfilter({'Column1', 'Column2'});
